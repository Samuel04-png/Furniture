          ...
          <span key={lineIndex} className="block min-h-[0.98em]">
            {[...line].map((character, characterIndex) => (
              <span
                ...
                className={character === '.' ? 'text-tm-gold' : undefined}
              >
                {character}
              </span>
            ))}
...
export default function HeroSection() {
  ...
  const heroPoster = getWebsiteMediaItem(companySettings, 'homeHeroPoster');
    ...
    <section className="relative min-h-[calc(100vh-var(--tm-nav-height-mobile))] overflow-hidden bg-tm-charcoal text-tm-cream lg:min-h-[calc(100vh-var(--tm-nav-height))]">
      <div className="absolute inset-0 z-0">
        <video
          ...
          poster={heroPoster.image}
        >
          <source
            src={asset(
              'videos/Behind every finished piece are skilled hands doing the work.This is Sikale, in motion - proudly.mp4',
            )}
            type="video/mp4"
          />
        </video>
          ...
          <div className="flex flex-col justify-center py-20 lg:py-32">
            ...
            <Reveal delay={0.6}>
              <p className="mt-8 max-w-[460px] font-dm text-[clamp(16px,1.1vw,18px)] font-light leading-[1.8] text-tm-cream/60">
                Bespoke hardwood furniture, handcrafted in Zambia from the finest local timber and
                tailored to the exact rhythm of your room.
              </p>
            </Reveal>

            <Reveal delay={0.8} className="mt-12 flex flex-col gap-4 sm:flex-row sm:gap-6">
              <Button to="/configure" variant="primary" className="px-10 py-5">
                Start Configuring
              </Button>
